...
if page == "Головна":
    ...
    * **Глобальність Instagram:** Рядок Instagram має найяскравіші кольори майже в усіх стовпчиках.
    ...
    st.write("---")
    st.subheader("📊 Профіль залежності за регіонами")
    st.write("Який відсоток студентів у кожному регіоні має високий рівень залежності?")

    # 1. Готуємо дані: рахуємо кількість для кожної пари Регіон-Рівень
    level_data = df.groupby(['Region', 'Addiction_Level']).size().reset_index(name='Students_Count')

    # 2. Розраховуємо відсотки всередині кожного регіону
    region_totals = level_data.groupby('Region')['Students_Count'].transform('sum')
    level_data['Percentage'] = (level_data['Students_Count'] / region_totals * 100).round(1)
    ...
    fig_levels = px.bar(
        ...
        y="Percentage",
        color="Addiction_Level",
        title="Співвідношення рівнів залежності (%)",
        labels={"Percentage": "Частка респондентів (%)", "Region": "Регіон"},
        color_discrete_map={
            "Low": "#2ca02c",    # Зелений
            "Medium": "#ff7f0e", # Помаранчевий
            "High": "#d62728"    # Червоний
        },
        # Сортуємо рівні від низького до високого
        category_orders={"Addiction_Level": ["Low", "Medium", "High"]},
        text=level_data['Percentage'].apply(lambda x: f'{x}%') # Виводимо % на стовпчиках
    )

    fig_levels.update_layout(
        barmode='stack', # Робимо стовпчики один на одному
        legend_title="Рівень залежності",
        yaxis_ticksuffix="%"
    )
    ...
    st.warning("""
    **Критичний інсайт:** Зверніть увагу на червоні зони. Якщо в регіоні з великою вибіркою (наприклад, Північна Америка) частка 'High' перевищує 30%, це вказує на системну проблему цифрової гігієни в цьому середовищі.
    """)
