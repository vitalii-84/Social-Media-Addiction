...
if page == "Головна":
    ...
    **Вердикт:** Гіпотеза 7 частково підтверджена. Ми спостерігаємо найвищі рівні цифрової залежності 
    ...
    st.title("🤖 Машинне навчання: Цифровий профіль")
    st.write("""
    ...
    with st.container(border=True):
        ...
        with col_in1:
            usage = st.slider("Скільки годин на день ви проводите в соцмережах?", 0.0, 24.0, 5.0)
    ...
    if st.button("Визначити мій профіль", type="primary", width='stretch'):
        # Проста логіка імітації моделі на основі наших кластерів з DataLab
        # Розраховуємо умовний бал ризику (спрощена версія нашої моделі)
        risk_score = (usage * 0.4) + (mental * 0.3) + (7 - sleep) * 0.3
        
        st.write("---")
        st.subheader("Результат аналізу:")
        
        if usage > 8 or risk_score > 6:
            st.error("🔴 **Ваш профіль: Цифрова залежність**")
            st.warning("Ваші показники вказують на високий ризик. Рекомендуємо встановити ліміти на використання додатків та звернути увагу на гігієну сну.")
        elif usage > 4 or risk_score > 4:
            st.warning("🟡 **Ваш профіль: Група ризику**")
            st.info("Ви проводите значну частину часу в мережі. Це може почати впливати на вашу продуктивність.")
        else:
            st.success("🟢 **Ваш профіль: Збалансований користувач**")
            st.balloons()
            st.write("Вітаємо! Ви вдало поєднуєте цифрове та реальне життя.")
