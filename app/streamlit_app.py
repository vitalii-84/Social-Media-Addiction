...
if page == "Головна":
...
elif page == "Глобальна geography":
    st.title("🌍 Глобальна географія залежності")
    st.write("Аналіз поширення цифрової залежності у світі та порівняння макрорегіонів.")

    # 1. Підготовка даних для карти
    # Групуємо за країнами та рахуємо середнє
    map_data = df.groupby('Country')['Addicted_Score'].mean().reset_index()
    ...
    fig_map = px.choropleth(
        ...
        color="Addicted_Score",
        hover_name="Country",
        title="Середній бал залежності за країнами",
        color_continuous_scale="YlOrRd", # Жовтий -> Помаранчевий -> Червоний
        labels={"Addicted_Score": "Бал залежності"}
    )
    ...
    # 3. Гіпотеза 7: Порівняння регіонів
    st.subheader("Гіпотеза 7: Регіональні відмінності (Пн. Америка vs Європа)")
    
    region_stats = df.groupby('Region')['Addicted_Score'].mean().sort_values(ascending=False).reset_index()
    ...
    fig_region = px.bar(
        ...
        orientation='h',
        color="Addicted_Score",
        title="Рівень залежності за макрорегіонами світу",
        labels={"Region": "Регіон", "Addicted_Score": "Середній бал"},
        color_continuous_scale="Viridis"
    )
    st.plotly_chart(fig_region, width='stretch')

    st.success("""
    **Вердикт:** Дані підтверджують Гіпотезу 7. Регіони з високою концентрацією технологічних хабів 
    (зокрема Північна Америка) демонструють дещо вищі показники адиктивності порівняно з Європою.
    """)
    ...
    st.info("Інтерактивний блок для визначення вашого цифрового профілю.")
