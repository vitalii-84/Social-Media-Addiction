...
if page == "Головна":
    ...
    with st.container(border=True):
        ...
        with col_in1:
            ...
            # Встановлюємо значення за замовчуванням 8.0 (ваша норма)
            sleep = st.slider("Скільки годин ви зазвичай спите?", 0.0, 12.0, 8.0, step=0.25)
        
        with col_in2:
            # Встановлюємо значення за замовчуванням 8 (хороший стан)
    ...
    if st.button("Визначити мій профіль", type="primary", width='stretch'):
        # ОНОВЛЕНА ФОРМУЛА:
        # (10 - mental) та (8 - sleep) перетворюють позитивні показники на низький бал ризику
        risk_score = (usage * 0.4) + ((10 - mental) * 0.3) + ((8 - sleep) * 0.3)
        
        st.write("---")
        st.subheader("Результат аналізу:")
        
        # Висновки на основі розрахованого risk_score та вашої таблиці середніх показників
        if usage >= 6.0 or risk_score > 5.0:
            st.error("🔴 **Ваш профіль: Високий рівень залежності**")
            st.warning("Ваші показники збігаються з групою 'High Addiction'. Рекомендуємо встановити ліміти на використання додатків та звернути увагу на гігієну сну.")
        elif usage >= 4.0 or risk_score > 3.0:
            st.warning("🟡 **Ваш профіль: Середній рівень (Група ризику)**")
            st.info("Ви знаходитесь у зоні 'Medium Addiction'. Це може почати негативно впливати на вашу продуктивність та емоційний стан.")
        else:
            st.success("🟢 **Ваш профіль: Збалансований користувач**")
            st.balloons()
            st.write("Ваші показники відповідають групі 'Low Addiction'. Ви вдало поєднуєте цифрове та реальне життя.")
