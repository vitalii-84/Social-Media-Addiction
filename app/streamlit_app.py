...
if page == "Головна":
    ...
    **Вердикт:** Гіпотеза 7 підтверджена. Регіони з високою концентрацією технологічних хабів 
    ...
    st.subheader("📊 Матриця концентрації: Платформи vs Регіони")
    st.write("Цей графік показує, де саме зосереджені користувачі кожної окремої мережі.")

    # 1. Готуємо зведену таблицю (Pivot Table)
    # Рахуємо кількість студентів для кожної пари Платформа-Регіон
    ...
    # 2. Будуємо теплову карту за допомогою Plotly
    fig_heatmap = px.imshow(
        heatmap_data,
        labels=dict(x="Регіон", y="Платформа", color="Кількість студентів"),
        x=heatmap_data.columns,
        y=heatmap_data.index,
        color_continuous_scale="Viridis", # Гарна темно-синьо-жовта гама
        text_auto=True, # Виводимо цифри прямо в клітинках
        aspect="auto"
    )

    fig_heatmap.update_layout(
        xaxis_title="Регіон світу",
        yaxis_title="Соціальна мережа",
        height=600
    )
    ...
    * **Глобальність Instagram:** Рядок Instagram має найяскравіші кольори майже в усіх стовпчиках.
    ...
    st.subheader("🎯 Матриця концентрації: Де "живуть" платформи?")
    st.write("Розмір бульбашки відображає кількість користувачів у розрізі регіону та платформи.")
    ...
    st.info("""
    **Швидкий аналіз:**
    * **Домінанти:** Найбільші кола показують "ядро" аудиторії.
    * **Пусті зони:** Відсутність кіл (наприклад, WeChat в Європі) чітко вказує на регіональну обмеженість певних мереж.
    """)
